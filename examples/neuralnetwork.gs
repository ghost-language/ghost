...
for (epoch = 0; epoch < 4000; epoch++) {
    ...
    if (epoch % 1000 == 0) {
        console.log("epoch " + epoch.toString() + "  loss " + math.round(math.sum(error * error), 5).toString())
    }
}
...
for (i = 0; i < 4; i++) {
    console.log(inputs[i].toString() + " -> " + math.round(output[i][0], 3).toString() + "  (want " + targets[i][0].toString() + ")")
}
